...
const SCROLL_THRESHOLD_START = 260
const SCROLL_THRESHOLD_END = 300

const AnimatedSectionList = Animated.createAnimatedComponent(SectionList<Dish>)

const Page = () => {
    const { id } = useLocalSearchParams<{ id: string }>();
    const [activeCateory, setActiveCategory] = useState(0)
    ...
    const categoryTabStyle = useAnimatedStyle(() => {
        const opacity = interpolate(scrollOffset.value, [SCROLL_THRESHOLD_START, SCROLL_THRESHOLD_END], [0, 1], Extrapolation.CLAMP)
    ...
    // const onViewableItemsChanged = useRef<NonNullable<React.ComponentProps<typeof SectionList>['onViewableItemsChanged']>>(({ viewableItems, changed }) => {
    ...
    //         if (sectionIndex !== -1 && sectionIndex !== activeCateory) {
    ...
    return (
        <View style={styles.container}>
            <Animated.Image style={[styles.backgroundImage, parallaxStyle]} source={restaurant.image!} resizeMode={"cover"} />
            ...
            <Animated.View style={[styles.stickyTabsOverlay, categoryTabStyle, { top: insets.top + 64 }]}>
                <View style={styles.categoryTabsContainer}>
                    ...
                    >
                        {menu?.map((category, index) => (
                            <TouchableOpacity key={`sticky-${index}`}
                                onPress={() => handleCategoryPress(index)}
                                style={[styles.categoryTab, activeCateory === index && styles.categoryTabActive]}>
                                <Text style={[styles.categoryTabText, activeCateory === index && styles.categoryTabTextActive]}>{category.category}</Text>
                            </TouchableOpacity>
                        ))}
                    </ScrollView>
                </View>
            ...
            <AnimatedSectionList
                ...
                renderSectionHeader={({ section }: { section: any }) => <View style={styles.sectionHeader}>
                ...
                ListHeaderComponent={
                    <>
                        ...
                        <View style={styles.whiteContentContainer}>
                            <Svg height={30} width={width} viewBox={`0 0 ${width} 30`} style={{ position: "absolute", top: -29, left: 0, right: 0 }}>
                                <Path
